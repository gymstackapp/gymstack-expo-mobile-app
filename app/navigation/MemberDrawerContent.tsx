...
//   { icon: "bell-outline", label: "Notifications", screen: "MemberNotifications" },
//   { icon: "gift-outline", label: "Refer & Earn", screen: "MemberReferral" },
//   { icon: "account-circle-outline", label: "Profile", screen: "MemberTabs", tab: "Profile" },
// ];
...
//     if (activeRouteName !== "MemberTabs") return null;
//     const tabsRoute = props.state.routes.find((r) => r.name === "MemberTabs");
...
//     if (item.screen === "MemberTabs") {
//       return activeRouteName === "MemberTabs" && activeTab === item.tab;
//     }
//     return activeRouteName === item.screen;
//   };

//   const onPress = (item: NavItem) => {
//     if (item.screen === "MemberTabs" && item.tab) {
//       (navigation as any).navigate("MemberTabs", { screen: item.tab });
//     } else {
...
//         <Avatar name={profile?.fullName ?? "M"} url={profile?.avatarUrl} size={34} />
    ...
    >
      ...
      >
        ...
        {NAV_ITEMS.map((item, i) => {
          const active = isActive(item);
          const showDivider = i > 0 && NAV_ITEMS[i - 1].label === DIVIDER_AFTER;
          return (
